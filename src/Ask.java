...
public class Ask extends QuickMath{
	...
	static boolean isExpr(String s) {
		return Interpreter.containsOperators(s) || s.equals("x") || Character.isDigit(s.charAt(0)) || s.equals("i") || s.equals("e");
	}
	...
	static void reformulate(ArrayList<String> tokens) {
		...
		for(int i = tokens.size()-1;i>=0;i--) {
			if(tokens.get(i).equals("squared")) {
				...
				tokens.remove(i);
			}else if(tokens.get(i).equals("log") || tokens.get(i).equals("ln")) {
				tokens.set(i, "ln("+tokens.get(i+1)+")");
				tokens.remove(i+1);
			}else if(tokens.get(i).equals("squareroot") || tokens.get(i).equals("sqrt")) {
				tokens.set(i, "sqrt("+tokens.get(i+1)+")");
				tokens.remove(i+1);
			}else if(tokens.get(i).equals("cuberoot") || tokens.get(i).equals("cbrt")) {
				tokens.set(i, "cbrt("+tokens.get(i+1)+")");
				tokens.remove(i+1);
			}else if(tokens.get(i).equals("root")) {
				if(tokens.get(i-1).equals("square")) {
					tokens.set(i, "sqrt("+tokens.get(i+1)+")");
					tokens.remove(i+1);
					tokens.remove(i-1);
					i++;
				}else if(tokens.get(i-1).equals("cube")) {
					tokens.set(i, "cbrt("+tokens.get(i+1)+")");
					tokens.remove(i+1);
					tokens.remove(i-1);
					i++;
				...
				if(indexOfExpr != -1) {
					tokens.set(i, "approx("+tokens.get(indexOfExpr)+")");
	...
	public static Expr ask(String question,Defs defs,Settings settings) {
		...
		if(tokens.contains("add") || tokens.contains("sum")) {
			Sum out = new Sum();
			for (int i:indexes) {
				Expr toBeAdded = createExpr(tokens.get(i));
		...
		if(tokens.contains("plot")) {
			Expr exprToPlot = createExpr(tokens.get(indexes.get(0)));
			Plot.PlotWindowParams windowParams = null;
			
			if(indexes.size() == 2) {
				ExprList params = (ExprList)createExpr(tokens.get(indexes.get(1)));
			...
			return new ImgExpr(Plot.renderGraph(exprToPlot, windowParams, imageSize));
		...
		if(tokens.contains("about") || tokens.contains("creator")) {
			return var("Benjamin Currie @2021 v "+Main.VERSION+" , java runtime version: "+System.getProperty("java.version"));
		}
